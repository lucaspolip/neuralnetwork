...
The use of LLMs in predicting consumer preferences in the household cleaning supplies category using online reviews.

## Why LLMs?

*Large Language Models (LLMs)* are useful for research today due to their ability to process nuanced conversation brought about by the fact that they have been trained on masses of nuanced human language which allows them to parse through regular expression in large amounts of data and pick up on specific elements, classify them based on outlined criteria, and learn and predict relationships (Gururangan et al., 2020).

## Assembling a Dataset

Given the above use of LLMs and the research question outlined above, an LLM could be trained for product reviews in household cleaning supplies to pick up on (1) positive, negative or neutral sentiment, (2) mentioned features, (3) whether other products were mentioned or not (4) price satisfaction. At least 2-300 products should be used to extract their reviews, and these products should be evenly split across the number of stars (Ansari, Essegaier, & Kohli, 2000). The reason household cleaning supplies have been chosen as a category is because they are widely used, have a lot of reviews on online platforms, and tend to result in very particular and binary reviews.

## The Analysis

Once the dataset is complete, it will speak to what kind of products customers are more likely to prefer over others, which features in products are most mentioned when products are preferred or when they are disliked, and what the correlation of price to features to preference is. Once a model has been trained on existing reviews, product descriptions for new products can then be fed into the system for the system to predict whether those features, product category and price will result in generally favorable reviews from the consumer or not (Li, Goh, & Jin, 2020).

## The Limitations

Given the fact that a lot of online reviews rely on nuanced consumer back-and-forth which resembles conversation, it is important to address the fact that LLMs are not often able to pick up on this nuance unless specifically trained via reinforcement learning to understand human intent. Research found that when LLMs were tested to see whether they understood implied context, their performance became random (Ruis et al., 2022). Even when trained, their accuracy was at best 72 percent compared to the 86 percent that humans can achieve on average (Ruis et al., 2022). There is therefore an obvious issue where nuanced predictions are hard to achieve given the generalized and conversational language people use in product reviews.
...
Li, L., Goh, T. T., & Jin, D. (2020). How textual quality of online reviews affect classification performance: a case of deep learning sentiment analysis. Neural Computing and Applications, 32, pp. 4387-4415.

Ruis, L., Khan, A., Biderman, S., Hooker, S., Rocktäschel, T., & Grefenstette, E. (2022). Large language models are not zero-shot communicators. arXiv preprint ar



Note: For a bit of background how one would actually send texts to such a model in a research project -- For now, many LLMs cannot be run locally on computers because they are too large, but instead are commonly used via a browsers. In a research project, however, it would be much easier to query the underlying model via an API (Application Programming Interface; if you are interested in the topic, have a look at the recording of week 8 in MY472) which are also often offered by the providers. In a nutshell, querying the model via an API means to send the input text with a programming language such as R or Python over the internet and receive the answer of the model (e.g. the response text or a numerical embedding of the text -- the LLMs can also return only embeddings that could be used for own downstream tasks!) back directly in the programming language. This allows to send many smaller texts to the model via loops and to process their returns directly in the programming language rather than using a visual user interface in a browser.
